...
<style>
  ...
  * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
  ...
  .lr-action { display: flex; align-items: center; justify-content: space-between; width: 100%; text-align: left;
    background: #fff; border: 1.5px solid ${e}; color: ${e}; font-size: 13px; font-weight: 600;
    padding: 10px 14px; border-radius: 999px; cursor: pointer; transition: background .15s ease, color .15s ease; }
  .lr-action:hover { background: ${e}; color: #fff; }
  .lr-action:hover svg { color: #fff; }
  .lr-action svg { flex-shrink: 0; color: ${e}; margin-left: 8px; transition: color .15s ease; }
  .lr-action:first-child { background: ${e}; color: #fff; }
  .lr-action:first-child svg { color: #fff; }
  .lr-action:first-child:hover { background: ${n}; }
</style>
...
</div>`}};function b(a){var e;let t=document.currentScript;if(t!=null&&t.dataset[a])return t.dataset[a];let r=document.querySelector(`script[data-${a.replace(/[A-Z]/g,n=>"-"+n.toLowerCase())}]`);return(e=r==null?void 0:r.dataset[a])!=null?e:null}function $(){return b("widgetKey")}function C(){return b("apiUrl")||g}async function I(a,t){try{let r=await fetch(`${t}/public/widget/config?widgetKey=${encodeURIComponent(a)}`);return r.ok?(await r.json()).data:null}catch(r){return null}}async function L(a,t,r,e,n){var i,s;try{let o=await fetch(`${a}/public/widget/chat?widgetKey=${encodeURIComponent(t)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sessionId:r,visitorId:e,message:n,pageUrl:location.href})}),d=await o.json();return!o.ok||!d.success?d.message||"Sorry, I'm having trouble responding right now.":(s=(i=d.data)==null?void 0:i.response)!=null?s:"Sorry, I didn't catch that."}catch(o){return"Sorry, I'm having trouble connecting right now. Please try again shortly."}}async function f(){let a=$();if(!a){console.warn("[LeadRyze Widget] Missing data-widget-key on the <script> tag \u2014 widget not loaded.");return}let t=C(),r=await I(a,t);if(!r){console.warn("[LeadRyze Widget] Could not load widget config (disabled, unknown key, or disallowed origin) \u2014 widget not loaded.");return}let e=u(),n=x(),i=new c(r,async s=>{i.setBusy(!0);let o=await L(t,a,n,e,s);i.addMessage("assistant",o),i.setBusy(!1)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{f()}):f();})();
