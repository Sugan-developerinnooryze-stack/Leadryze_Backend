...
export async function list(req: AuthRequest, res: Response) {
  try {
    const { items, total, page } = await listAssets(req.tenantId!, req.query as any, req.branchId, resolveEffectiveScope(req, 'assets'));
    sendPaginated(res, items, total, page, Number(req.query.limit ?? 20));
  } catch (err: any) { sendError(res, err.message, 500); }
}
export async function getOne(req: AuthRequest, res: Response) {
  try {
    const item = await getAssetById(req.params.id, req.tenantId!, resolveEffectiveScope(req, 'assets'));
...
export async function update(req: AuthRequest, res: Response) {
  try {
    const item = await updateAsset(req.params.id, req.tenantId!, req.body, resolveEffectiveScope(req, 'assets'));
    if (!item) return sendError(res, 'Asset not found', 404);
    sendSuccess(res, item);
  } catch (err: any) { sendError(res, err.message, 400); }
}
export async function remove(req: AuthRequest, res: Response) {
  try {
    const item = await deleteAsset(req.params.id, req.tenantId!, resolveEffectiveScope(req, 'assets'));
    if (!item) return sendError(res, 'Asset not found', 404);
