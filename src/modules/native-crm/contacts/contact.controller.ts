...
export async function getOne(req: AuthRequest, res: Response) {
  try {
    const record = await svc.getContactById(req.tenantId!, req.params.id, resolveEffectiveScope(req, 'contacts'));
...
export async function remove(req: AuthRequest, res: Response) {
  try {
    const ok = await svc.deleteContact(req.tenantId!, req.params.id, resolveEffectiveScope(req, 'contacts'));
...
export async function stats(req: AuthRequest, res: Response) {
  try { sendSuccess(res, await svc.getContactStats(req.tenantId!, resolveEffectiveScope(req, 'contacts'))); }
