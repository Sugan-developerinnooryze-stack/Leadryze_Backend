...
export async function list(req: AuthRequest, res: Response) {
  try {
    const { items, total, page } = await listVehicles(req.tenantId!, req.query as any, req.branchId, resolveEffectiveScope(req, 'vehicles'));
    sendPaginated(res, items, total, page, Number(req.query.limit ?? 20));
  } catch (err: any) { sendError(res, err.message, 500); }
}
export async function getOne(req: AuthRequest, res: Response) {
  try {
    const item = await getVehicleById(req.params.id, req.tenantId!, resolveEffectiveScope(req, 'vehicles'));
...
export async function update(req: AuthRequest, res: Response) {
  try {
    const item = await updateVehicle(req.params.id, req.tenantId!, req.body, resolveEffectiveScope(req, 'vehicles'));
    if (!item) return sendError(res, 'Vehicle not found', 404);
    sendSuccess(res, item);
  } catch (err: any) { sendError(res, err.message, 400); }
}
export async function remove(req: AuthRequest, res: Response) {
  try {
    const item = await deleteVehicle(req.params.id, req.tenantId!, resolveEffectiveScope(req, 'vehicles'));
    if (!item) return sendError(res, 'Vehicle not found', 404);
